...
from ..util.email.SendMessage import SendMessage
...
def aps_test(project_id, task_name, case_ids, send_address=None, send_password=None,
             task_to_address=None, performer='无'):
    # global db
    # db.session.remove()
    ...
    jump_res = d.run_case()
    d.build_report(jump_res, task_name, case_ids, performer)
    ...
    if send_address:
        ...
        start_at = time.strftime("%Y--%m--%d %H:%M:%S", time.localtime(res['time']['start_at']))
        duration = "{:.3f}".format(res['time']['duration'])
        result_data = "Hi, all:\n"  \
                      "    测试定时任务：%s\n" \
                      "    总执行测试接口数： %s, 用时：%s s\n" \
                      "    成功：%s, 失败：%s, 错误：%s, 跳过：%s\n" \
                      "    测试时间：%s\n" \
                      "    详情查看地址：http://172.20.11.210/#/manage/reportManage\n" \
                      "    查看使用用户名密码：test/test" \
...
def run_task():
    ...
    new_report_id = aps_test(_data.project_id, _data.task_name, cases_id,
...
def start_task():
    ...
    scheduler.add_job(func=aps_test, trigger='cron', misfire_grace_time=60, coalesce=False,
                      args=[_data.project_id, _data.task_name, cases_id, _data.task_send_email_address,
                            _data.email_password, _data.task_to_email_address,
...
def add_task():
    ...
    if not (not send_email and not password) and not (send_email and password):
        return jsonify({'msg': 'Webhook、安全加签必须都为空，或者都必填写', 'status': 0})
