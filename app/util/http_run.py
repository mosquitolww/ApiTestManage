...
class RunCase(object):
    ...
    def build_report(self, jump_res, task_name, case_ids, performer):
        # if self.run_type and self.make_report:
        res = json.loads(jump_res)
        testcases = res['stat']['testcases']
        teststeps = res['stat']['teststeps']
        new_report = Report(performer=performer,
                            ...
                            case_run=testcases['total'],
                            case_successes=testcases['success'],
                            api_run=teststeps['total'],
                            api_successes=teststeps['successes'],
    ...
    def run_case(self):
        ...
        # self.TEST_DATA = {'project_mapping': {'env': {'USERNAME': 'leolee', 'PASSWORD': '123456'}, 'test_path':
        # 'D:\\project\\qwe\\demo\\er\\do\\testsuites\\demo_testsuite.yml', 'PWD': 'D:\\project\\qwe\\demo\\er\\do'},
        # 'testsuites': [{'testcases': {'call demo_testcase with data 1': {'testcase': 'testcases/demo_testcase.yml',
        # 'name': 'call demo_testcase with data 1', 'testcase_def': {'config': {
        # 'variables': {'device_sn': 'XYZ', 'passwords': 123456789},
        # 'name': 'demo testcase', 'base_url': 'http://122.51.184.120', 'parameters':
        # {'passwords': [123456789, 1213456789, 1223456789]}},
        # 'teststeps': [{'variables': {'user_agent': 'iOS/10.3'},
        # 'api': 'api/api1.yml', 'name': 'demo step 1', 'api_def': {'variables': {'var2': 'value2', 'var1': 'value1'},
        # 'name': 'demo api', 'validate': [{'eq': ['status_code', 200]}],
        # 'request': {'url': '/api/login', 'method': 'POST',
        # 'data': {'account': 'admin', 'password': '$passwords'}}}, 'validate': [{'eq': ['status_code', 200]}]}]},
        # 'parameters': {'passwords': [123456789, 1213456789, 1223456789]}}}, 'config': {
        # 'variables': {'device_sn': 'XYZ', 'passwords': 123456789}, 'name': 'demo testsuite',
        # 'base_url': 'http://122.51.184.120'},
        # 'path': 'D:\\project\\qwe\\demo\\er\\do\\testsuites\\demo_testsuite.yml', 'type': 'testsuite'}]}
